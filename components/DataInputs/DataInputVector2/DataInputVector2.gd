...
func _ready() -> void:
	...
	x_axis.value_changed.connect(func (_p_text): _make_unsaved())
	x_axis.get_line_edit().text_changed.connect(func (_p_text): _make_unsaved())
	x_axis.get_line_edit().gui_input.connect(_on_axis_gui_input)
	
	y_axis.value_changed.connect(func (_p_text): _make_unsaved())
	y_axis.get_line_edit().text_changed.connect(func (_p_text): _make_unsaved())
...
func _settings_module_changed(p_module: SettingsModule) -> void:
	var min_val: Vector2 = type_convert(p_module.get_min(), TYPE_VECTOR2)
	var max_max: Vector2 = type_convert(p_module.get_max(), TYPE_VECTOR2)
	
	x_axis.min_value = min_val.x
	x_axis.max_value = max_max.x
	
	y_axis.min_value = min_val.y
	y_axis.max_value = max_max.y
