...
func _process(_p_delta: float) -> void:
	var current_size: int = int(get_combined_minimum_size().x)
	var new_size: int = current_size + int(get_global_mouse_position().x - _previous_pos)
	
	if new_size == current_size:
		return
	
	set_min_width(new_size)
	_previous_pos = int(get_global_mouse_position().x)
